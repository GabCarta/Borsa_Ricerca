"""Pagina che contiene l'interfaccia grafica relativa al modulo Send Data,
che permette di inviare telemetrie ai dispositivi registrati"""
...
with st.container(border=True):
    ...
    device_id = st.text_input("Target Device ID", value="termostato_1")

    st.subheader("3. Payload Builder")
    c1, c2 = st.columns(2)
    invia_stato = c1.checkbox("Include 'State'", value=True)
    valore_stato = c1.selectbox("State Value", ["ON", "OFF"], disabled=not invia_stato)

    invia_consumo = c2.checkbox("Include 'Consumption'", value=True)
    ...
    for i in range(int(num_extra)):
        ...
        p_name = cA.text_input(f"Name {i+1}", key=f"dyn_name_{i}")
        p_val = cB.text_input(f"Value {i+1}", key=f"dyn_val_{i}")
        p_type = cC.selectbox("Type", ["float", "int", "string"], key=f"dyn_type_{i}")
        ...
        if p_name and p_val:
            try:
                ...
                st.error(f" Impossibile convertire '{p_val}' in {p_type}.")

    st.write("") 
    btn_label = "Invia Comando"
    ...
    if not security_key:
        ...
        try:
            ...
            if risposta.status_code == 200:
                ...
                st.error(f" API Error: {risposta.status_code} - Details: {risposta.text}")
        except Exception:
